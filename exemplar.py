# train Dict[str,List] Dictionary maintaining the training samples
# val = Dict[str,List] Dictionary maintaining the validation samples
...
class Exemplar:
    ...
    def update(self, cls_num, train, val):
        ...
        #updating the current classes by adding the new number of classes
        self.cur_cls += cls_num
        # total_store_num  = average number of data samples per class
        total_store_num = self.max_size / self.cur_cls if self.cur_cls != 0 else max_size
        # splitting the data in train and test in 90:10 ratio
